...
DATA_PATH = "bitirme-projesi/tahmin_csv_dosyalari/grace_tr_twsa_filled.csv"
WINDOW_SIZE = 24
EPOCHS = 100
FORECAST_STEPS = 60
...
def main():
    ...
    df_full_twsa = pd.read_csv("bitirme-projesi/tahmin_csv_dosyalari/grace_tr_twsa_filled.csv")
    ...
    ax.set_title(f"TWSA Forecast (Start: {forecast_start_date.strftime('%m.%Y')}) - {MODEL_NAME} (TWSA+ERA5)")
    ax.set_xlabel("Time")
    ...
    file_prefix = f"forecast_{MODEL_NAME.lower()}_era5_{forecast_start_date.strftime('%Y_%m')}"
