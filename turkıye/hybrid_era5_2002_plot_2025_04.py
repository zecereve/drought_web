...
DATA_PATH = "grace_tr_twsa_filled.csv"
WINDOW_SIZE = 24
EPOCHS = 100
FORECAST_STEPS = 60
...
def load_data():
    ...
    df.rename(columns={df.columns[0]: 'time', 'precip_mm': 'precip'}, inplace=True)
...
def build_model(window_size):
    ...
    if MODEL_NAME == "LSTM":
        ...
        # Hybrid LSTM + GRU model combining both temporal aspects
...
def main():
    ...
    # Tüm gerçek TWSA verisini tam çizmek için:
    df_full_twsa = pd.read_csv(DATA_PATH)
    df_full_twsa.rename(columns={df_full_twsa.columns[0]: 'time'}, inplace=True)
    ...
    ax.plot(df_full_twsa.index, df_full_twsa['twsa'], color="black", linewidth=2.8, alpha=0.55, label="Observed TWSA")
    ...
    overlap_idx_plot = future_df['twsa_forecast'].dropna().index.intersection(df_full_twsa.index)


    ax.axvline(forecast_start_date, linestyle="--", linewidth=2, color="gray", label=f"Start: {forecast_start_date.strftime('%m.%Y')}")
    ax.set_title(f"TWSA Forecast (Start: {forecast_start_date.strftime('%m.%Y')}) - {MODEL_NAME} (TWSA+ERA5) - From 2002")
    ax.set_xlabel("Time")
    ...
    try:
        # Changed here to start from 2002
        ax.set_xlim([pd.to_datetime("2002-04-01"), future_df.index.max()])
    except:
        pass
    fig.tight_layout()

    file_prefix = f"forecast_{MODEL_NAME.lower()}_era5_2002_{forecast_start_date.strftime('%Y_%m')}"
    future_df.to_csv(f"{file_prefix}.csv")

    fig.savefig(f"{file_prefix}.png", dpi=300)
    plt.close(fig)

    # Yıllık grafikleri çizdirme (2025-2030)
    ...
    for year in target_years:
        ...
        mask_obs = (df_full_twsa.index >= start_of_year) & (df_full_twsa.index <= end_of_year)
        if mask_obs.any():
            ax_y.plot(df_full_twsa.index[mask_obs], df_full_twsa.loc[mask_obs, 'twsa'], 
                      color="black", linewidth=2.8, alpha=0.55, label="Observed TWSA", marker='o')
        
        mask_pred = (future_df.index >= start_of_year) & (future_df.index <= end_of_year)
        if mask_pred.any():
            ax_y.plot(future_df.index[mask_pred], future_df.loc[mask_pred, 'twsa_forecast'], 
                      color="green", linewidth=3, label='Forecast', marker='o')
            
        ax_y.set_title(f"TWSA Forecast for {year} - {MODEL_NAME} (TWSA+ERA5)")
        ax_y.set_xlabel("Time")
        ax_y.set_ylabel("TWSA (cm)")
        if mask_obs.any() or mask_pred.any():
            ax_y.legend(loc="best")
        ax_y.grid(True, alpha=0.3)
        ax_y.set_xlim([start_of_year, end_of_year])
        
        fig_y.tight_layout()
        fig_y.savefig(f"{file_prefix}_year_{year}.png", dpi=300)
        plt.close(fig_y)
